...
public class AliseSearchPanel extends JPanel {
	...
	public AliseSearchPanel() {
		...
		var label_1 = new JLabel(rsc_bdl.getString("ALISE.SEARCH.PANEL.NAME"));
		var label_2 = new JLabel(rsc_bdl.getString("ALISE.SEARCH.PANEL.ADDRESS"));
		...
		var resolve_btn = new JButton(rsc_bdl.getString("ALISE.SEARCH.PANEL.RESOLVE"), UIUtil.getStretchIcon("icon/" + "arrow-down.svg", icon_size, icon_size));
		var r_resolve_btn = new JButton(rsc_bdl.getString("ALISE.SEARCH.PANEL.REVERSE_RESOLVE"), UIUtil.getStretchIcon("icon/" + "arrow-up.svg", icon_size, icon_size));
	...
	protected void start_resolve() {
		if (field_1.getText().isBlank()) {
			JOptionPane.showMessageDialog(getRootPane(), rsc_bdl.getString("ALISE.SEARCH.PANEL.NAME_CANNOT_BE_EMPTY"), rsc_bdl.getString("GENERAL.ERROR"), JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		Util.submit(() -> {
			...
			if (r == null) {
				...
				JOptionPane.showMessageDialog(getRootPane(), rsc_bdl.getString("ALISE.SEARCH.PANEL.CANNOT_LOAD_ENS"), rsc_bdl.getString("GENERAL.ERROR"), JOptionPane.ERROR_MESSAGE);
				return;
			}
	...
	protected void r_resolve_btn() {
		if (field_2.getText().isBlank()) {
			JOptionPane.showMessageDialog(getRootPane(), rsc_bdl.getString("ALISE.SEARCH.PANEL.ADDR_CANNOT_BE_EMPTY"), rsc_bdl.getString("GENERAL.ERROR"), JOptionPane.ERROR_MESSAGE);
			return;
		}
