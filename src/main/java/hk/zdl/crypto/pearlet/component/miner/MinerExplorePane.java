...
public class MinerExplorePane extends JTabbedPane {
	...
	private final class StartPanel extends JPanel {
					...
					SwingUtilities.invokeLater(() -> {
						try {
							iface_combobox.setModel(new ListComboBoxModel<Inet4Address>(NetworkInterface.networkInterfaces().filter(f -> !f.isVirtual()).flatMap(f -> f.inetAddresses())
									.filter(a -> !a.isAnyLocalAddress() && !a.isLinkLocalAddress() && !a.isLoopbackAddress()).filter(a -> a instanceof Inet4Address).map(a -> (Inet4Address) a)
									.toList()));
		...
		private void addMinerDetailPane(String base_path) throws Exception {
			...
			pane.setStatus(jobj);
			insertTab("Miner", getIcon(), pane, base_path, getTabCount());
		}
